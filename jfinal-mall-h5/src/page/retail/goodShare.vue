...
export default {
  ...
  methods: {
    ...
    // 查看商品
    Good(data) {
      sessionStorage.setItem("upper", "/retail/share?text=分享送积分&u=");
      this.$router.push({
    ...
    searchByNameFun(goodName = "") {
      ...
      let data = Object.assign(
        {
          isEarnPoint: that.goodType.isEarnPoint,
          pointAsMoney: that.goodType.pointAsMoney,
          categoryId: that.id,
          goodName: goodName ? goodName : that.search,
  ...
  created() {
    let qParam = this.$route.query;
    this.navbar.title.coment = qParam.text;
    if (!(qParam.from && qParam.from === "index")) {
      this.navbar.left[0].to = "/retail";
    }
    if (qParam.isEarnPoint && qParam.isEarnPoint === "1") {
      console.log("赚取积分的商品");
      this.goodType.isEarnPoint = 1
    }

    if (qParam.pointAsMoney && qParam.pointAsMoney === "true") {
      console.log("消费积分的商品");
      this.goodType.pointAsMoney = true
    }
  }
};
</script>
