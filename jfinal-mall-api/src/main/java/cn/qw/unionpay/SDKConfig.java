    ...
    public void loadProperties(Properties pro) {
        ...
        if (SDKUtil.isEmpty(value) || SDKConstants.TRUE_STRING.equals(value)) {
            ...
            if (!SDKUtil.isEmpty(value)) {
                this.signCertPath = PathKit.getRootClassPath() + value.trim();    // 使用相对路径
        ...
        if (!SDKUtil.isEmpty(value)) {
            this.encryptCertPath = PathKit.getRootClassPath() + value.trim();    // 使用相对路径
            LogUtil.writeLog("配置项：敏感信息加密证书==>" + SDK_ENCRYPTCERT_PATH + "==>" + value + " 已加载");
        }
        value = pro.getProperty(SDK_VALIDATECERT_DIR);
        if (!SDKUtil.isEmpty(value)) {
            this.validateCertDir = PathKit.getRootClassPath() + value.trim();
        ...
        if (!SDKUtil.isEmpty(value)) {
            this.encryptTrackCertPath = PathKit.getRootClassPath() + value.trim();    // 使用相对路径
        }
